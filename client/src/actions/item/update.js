...
export function retrieveError(retrieveError) {
  return { type: 'ITEM_UPDATE_RETRIEVE_ERROR', retrieveError };
}

export function retrieveLoading(retrieveLoading) {
  return { type: 'ITEM_UPDATE_RETRIEVE_LOADING', retrieveLoading };
}

export function retrieveSuccess(retrieved) {
  return { type: 'ITEM_UPDATE_RETRIEVE_SUCCESS', retrieved };
}
...
export function updateError(updateError) {
  return { type: 'ITEM_UPDATE_UPDATE_ERROR', updateError };
}

export function updateLoading(updateLoading) {
  return { type: 'ITEM_UPDATE_UPDATE_LOADING', updateLoading };
}

export function updateSuccess(updated) {
  return { type: 'ITEM_UPDATE_UPDATE_SUCCESS', updated };
}
...
export function mercureOpen(eventSource) {
  return { type: 'ITEM_UPDATE_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'ITEM_UPDATE_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'ITEM_UPDATE_MERCURE_MESSAGE', retrieved });
  };
}
